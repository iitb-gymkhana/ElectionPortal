...
def authenticate_code(request):
    ...
    if code is None:
        ...
        if user:
            ...
            login_complete_redirect = oauth_settings[
                'LOGIN_COMPLETE_REDIRECT'] if 'LOGIN_COMPLETE_REDIRECT' in oauth_settings else '/'
            return redirect(login_complete_redirect)
        else:
            print("Not logged in, redirecting")
            alternate_login = get_django_setting_or_default(
                'ALTERNATE_LOGIN_URL', '/')
