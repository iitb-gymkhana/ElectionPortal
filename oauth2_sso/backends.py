...
class OAuth2Backend(ModelBackend):
    ...
    CLIENT_SECRET_BASE64 = _get_oauth_setting_or_none('CLIENT_SECRET_BASE64')
    # User definition stuff (optional)
    USER_GROUPS_URL = _get_oauth_setting_or_none('USER_GROUPS_URL')
    GROUP_EXTRACTION_FUNCTION = _get_oauth_setting_or_none(
        'GROUP_EXTRACTION_FUNCTION')
    USER_CREATION_FUNCTION = _get_oauth_setting_or_none(
    ...
    def authenticate(self, request=None, code=None, **kwargs):
        ...
        data = 'code=' + code + '&redirect_uri=' + \
            self.REDIRECT_URI + '&grant_type=' + self.GRANT_TYPE

        # credentials = self.CLIENT_ID + ':' + self.CLIENT_SECRET
        # auth = base64.b64encode(credentials)

        auth_response = requests.post(self.TOKEN_URL, data=data, headers={
            "Authorization": "Basic " + base64.b64encode(f"{self.CLIENT_ID}:{self.CLIENT_SECRET}"
        ...
        if 'access_token' not in auth_response.json():
            return Response(auth_response.json(), status=400)
        else:
    ...
    def configure_user(self, user, user_info):
        ...
        user.save(update_fields=list(
            map(lambda x: x[1], self.USER_FIELD_MAPPINGS)))
        return user

    def get_or_create_user(self, user_info):
        try:
            user = self.UserModel._default_manager.get_by_natural_key(
                user_info[self.USERNAME_FIELD])
    ...
    def setup_user_groups(self, user):
        ...
        if user_groups_response.status_code == 200:
            groups = import_from(self.GROUP_EXTRACTION_FUNCTION)(
                self.request, user, user_groups_response.json())
