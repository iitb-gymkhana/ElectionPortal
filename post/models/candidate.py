...
class Candidate(models.Model):
    ...
    image = models.ImageField(
        upload_to=candidate_image_upload, max_length=512, null=True, blank=True)
    manifesto = models.FileField(
        upload_to=candidate_image_upload, max_length=512, null=True, blank=True)
    post = models.ForeignKey(
        Post, related_name='candidates', db_index=True, on_delete=models.CASCADE)
    order = models.IntegerField(
        default=0, help_text='Sorting order of candidates')
